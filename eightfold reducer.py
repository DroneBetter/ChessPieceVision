class reducer:
    ...
    def conditionalReverse(self,reversal,toBe):
        return reversed(toBe) if reversal else toBe
    def conditionalFlip(self,flip,toBe,transitive=False):
    ...
    def conditionalTranspose(self,transpose,x,y): #to prevent having to reiterate in each instance or use the less efficient x*boardWidth**(transpose)+y*boardWidth**(1^transpose) (to avoid having to allocate x and y to variables or recompute them)
    ...
    def RLE(self,board,includeRule=False):
    ...
    def positionReflect(self,position,axis):
    ...
    def compoundPositionReflect(self,position,axes,reverse=False): #some are probably reducible further (depending on whether floor division or modulo is more efficient, but also probably regardless)
        return ( ( ( ( (position+1)*boardWidth+~((boardSquares+1)*(position//boardWidth)))
                 ...
                 ( ( (boardSquares+~position) #(boardWidth-position//boardWidth)*boardWidth+(boardWidth-position%boardWidth)=boardSquares+boardWidth-position
                  ...
                  if axes[1] else
                   ( boardWidth*(1+2*(position//boardWidth))+~position #position=position%boardWidth+position//boardWidth*boardWidth, so position//boardWidth*boardWidth=position-position%boardWidth, so boardWidth+~position%boardWidth+position//boardWidth*boardWidth=boardWidth+~position%boardWidth+position-position%boardWidth=boardWidth+~2*position%boardWidth+position=boardWidth+~(2*position%boardWidth)+position=~position+boardWidth*(1+2*position//boardWidth)
                    if axes[0] else
                     position)))
    def boardReflect(self,board,axis):
    ...
    def compoundReflect(self,board,axes):
        return board if axes==(0,0,0) else tuple(x for y in self.conditionalReverse(axes[not axes[2]],range(boardWidth)) for x in self.conditionalReverse(axes[axes[2]],(board[y:y+boardSquares:boardWidth] if axes[2] else board[y*boardWidth:(y+1)*boardWidth])))
    def layerIndices(self,x,y):
    ...
    def layerType(self,x,y):
        return ((1 if x==0 else 2 if y==0 else 3 if y==x else 0) if boardWidth%2 else (3 if y==x else 0))
    def layerPriorities(layer,type):
        return(self.instanceReflectionPriorities[type][sum(l<<i for i,l in enumerate(layer))])
    ...
    def symmetry(self,state,reflectionMode=0):
        if self.symmetryReduction:
            layers=[list(map(state.__getitem__,l)) for l in unreflectedLayerIndices]
            ...
            while l<len(layers):
                ...
                newReflections=reflections&sum((p==m)<<i for i,p in enumerate(self.instanceReflectionPriorities[layerTypes[l]][layerNumbers[l]])) #may produce multiple but they will all yield the same board
                reflections=newReflections
    ...
    def generateCellular(self):
        if self.symmetryReduction:
            ...
            while True:
                arm=len(layerNumbers)-1
                states.append(self.constructCellular(layerNumbers))
                while True:
                    ...
                    if arm<0:
                        ...
                        #print(tuple(map(bin,branchReflections)),self.instanceReflectionPriorities[layerTypes[arm]][layerNumbers[arm]],sum((p==m)<<i for i,p in enumerate(self.instanceReflectionPriorities[layerTypes[arm]][layerNumbers[arm]]) if arm==0 or branchReflections[-2]>>i&1))
                ...
                if arm<0:
                        break
                branchReflections[-1]=((1<<8)-1 if arm==0 else branchReflections[-2])&sum((p==m)<<i for i,p in enumerate(self.instanceReflectionPriorities[layerTypes[arm]][layerNumbers[arm]]) if arm==0 or branchReflections[-2]>>i&1) #formerly &ed by instanceReflectionAllowednesses[layerTypes[arm]][layerNumbers[arm]]
            ...
            return(tuple(tuple(i>>n&1 for n in range(boardSquares)) for i in range(2**boardSquares)))
    """for i in range(1,8):
        setBoardWidth(i)
            ...
            while l<len(layers):
                for n in range(layerNumbers[l]+1):
                    ...
                    if self.instanceReflectionPriorities[layerTypes[l]][n][0]==m: #legal
                        newReflections=reflections&sum((p==m)<<i for i,p in enumerate(self.instanceReflectionPriorities[layerTypes[l]][n])) #may produce multiple but they will all yield the same board
                        if n<layerNumbers[l]:
        ...
        if self.symmetryReduction:
            ...
            while l<len(layerNumbers):
                for n in range(2**self.typeLengths[layerTypes[l]]):
                    m=min(p for i,p in enumerate(self.instanceReflectionPriorities[layerTypes[l]][n]) if reflections>>i&1)
                    if self.instanceReflectionPriorities[layerTypes[l]][n][0]==m: #legal
                        newNewReflections=reflections&sum((p==m)<<i for i,p in enumerate(self.instanceReflectionPriorities[layerTypes[l]][n])) #may produce multiple but they will all yield the same board
    ...
    def __init__(self,boardWidth,reduction=True):
        ...
        instanceReflectionAllowednesses=[[sum((l==0)<<i for i,l in enumerate(j)) for j in i] for i in self.instanceReflectionPriorities]
        ...
        equation("self.instanceReflectionPriorities",tuple(tuple(tuple(j>>(typeBits[m]*l)&(typeLengths[m]-1) for l in range(8)) for j in i) for m,i in enumerate(((sum(k<<(typeBits[m]*l) for l,k in enumerate(j)) for j in i) for m,i in enumerate(self.instanceReflectionPriorities)))),3)
        equation("self.instanceReflectionPriorities",((sum(k<<(typeBits[m]*l) for l,k in enumerate(j)) for j in i) for m,i in enumerate(self.instanceReflectionPriorities)),2)
        print(tuple(map(lambda n: tuple(map(tuple,n)),self.instanceReflectionPriorities))==tuple(tuple(tuple(j>>(typeBits[m]*l)&(typeLengths[m]-1) for l in range(8)) for j in i) for m,i in enumerate(((sum(k<<(typeBits[m]*l) for l,k in enumerate(j)) for j in i) for m,i in enumerate(self.instanceReflectionPriorities)))))'''
...
#import pprint;pprint.pprint(self.instanceReflectionPriorities);pprint.pprint(instanceReflectionAllowednesses)
from itertools import groupby
from functools import reduce
