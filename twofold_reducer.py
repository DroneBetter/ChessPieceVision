...
#these parts from https://github.com/DroneBetter/DroneLambda/blob/main/DroneLambda.py but without right-associativity
...
class twofold:
    ...
    def __init__(self,n,symmetry=True,scroll=False,subperiods=True):
        ...
        self.reverse=reversalParameters(n)
        if scroll:
    ...
    symmetry=(lambda self,s: min(map(lambda s: min(redumulate(lambda s,i: s>>1|(s&1)<<self.n-1,range(self.n),s)),((s,self.reverse(s)) if self.sym else (s,)))) if self.scroll else funcxp(self.reverse,(lambda b: b<=self.n//2 and self.layer(s,b)==2)(self.branch(s)))(s) if self.sym else s)
        ...
        if self.scroll:
            while True: #runs for asymptotically constantly many iterations before termination
                ...
                if t<self.n:
                    w>>=t
                    l=self.n-t
                    w=reduce(lambda w,i: w|w>>(l<<i),range(l and self.n//l),(w^1)<<self.n-l)
                    if (not self.n%l if self.subperiods else self.n==l) and (not self.sym or self.canonical(w)):#self.canonical(w,w.bit_length())):
                        return(w,l)
                else:
                    raise(StopIteration)
        else:
    ...
    __iter__=(lambda self,l=None,u=None: map(lambda i: i[0],redumulate(lambda s,i: self.nexter(*s),range((len(self) if u==None else u if l==None else u-l)-1),(0,1 if self.scroll else self.n) if self.scroll or l==None else (lambda s: (s,self.branch(s)))(self[l]))) if self.sym or self.scroll else iter(range(len(self)) if l==None else range(l) if u==None else range(l,u))) #does not compute branch
...
if __name__=='__main__':
    ...
    #print((lambda n: '\n'.join(map(n.strate,n)))(twofold(11,False,True)))
    #print((lambda n: '\n'.join(map(n.strate,n)))(twofold(11,True,True)));exit()
